...
public class SystemAuthorizingRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken) {
		...
		if(StringUtils.isNotBlank(token.getErrorMsg())){
			throw new AuthenticationException(token.getErrorMsg());
		}
...
//		}
		if (user != null) {
			if (Global.NO.equals(user.getLoginFlag())) {
				throw new AuthenticationException("msg:该帐号已被禁用，请联系管理员");
			}
			// web登陆
			// 判断是否是系统内置超级管理员,若是,就不从app获取token,使用本地数据库鉴权(超级管理员只能在web端登陆)
			if (UserUtils.SUPER_ADMIN_USERNAME.equals(user.getLoginName())) {
				setCredentialsMatcher(new SystemAdminCredentialMatcher());
				byte[] salt = Encodes.decodeHex(user.getPassword().substring(0, 16));
				return new SimpleAuthenticationInfo(new Principal(user, token.isMobileLogin()),
						user.getPassword().substring(16), ByteSource.Util.bytes(salt), getName());
			} else {
				setCredentialsMatcher(new WebCredentialsMatcher());
				String cacheToken = UserUtils.getTokenById(user.getId());
				return new SimpleAuthenticationInfo(new Principal(user, token.isMobileLogin()),
						cacheToken, getName());
			}
		} else {
			throw new AuthenticationException("crm还未同步该用户,请稍后重试");
		}
	}
