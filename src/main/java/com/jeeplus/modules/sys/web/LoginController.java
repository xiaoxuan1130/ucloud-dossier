...
import com.epipe.ucloud.dossier.app.entity.AppUrlProperty;
import com.epipe.ucloud.dossier.app.service.AppInterfaceService;
import com.epipe.ucloud.dossier.utils.QRCodeUtils;
...
public class LoginController extends BaseController{
	...
	public String loginFail(HttpServletRequest request, HttpServletResponse response, Model model) {
		...
		// 如果是手机登录，则返回JSON字符串
		if (mobile||(StringUtils.isNotBlank(requestType))&&requestType.equals("1")){
			AjaxJson j = new AjaxJson();
			j.setSuccess(false);
			j.setMsg(message);
	...
	public String index(HttpServletRequest request, HttpServletResponse response) {
		...
		if(UserUtils.getMenuList().size() == 0){
			...
			if(MobileUtils.checkAgentIsMobile(request.getHeader("User-Agent"))){//如果是移动，默认使用左侧菜单，显示效果较好
				indexStyle = "ani";
			}else{//如果是pc端，默认使用epipe.properties中的配置项，jp横向菜单，ani左侧菜单
			...
			if (StringUtils.isNotEmpty(indexStyle)
				...
				//return "modules/sys/firstPage/firstPageForm";
			}
	 ...
	 * @return
	 */
	@RequestMapping(value = "${adminPath}/createCode")
	@ResponseBody
	public void createCode(String code,HttpServletRequest request, HttpServletResponse response) {
		String appUrl=appUrlProperty.getAppUrl();
		QRCodeUtils.creatRrCode(appUrl,code, 200,200,response);
	}

	@RequestMapping(value = "${adminPath}/getuuid")
	@ResponseBody
	public AjaxJson getAppUUID(){
		String result=appInterfaceService.getAppUuid();
		AjaxJson j = new AjaxJson();
		j.setSuccess(true);
		j.setMsg(result);
		return j;
	...
	@RequestMapping(value = "${adminPath}/getstatus")
	@ResponseBody
	public AjaxJson getStatus(String code){
		String result=appInterfaceService.getAppStatus(code);
		AjaxJson j = new AjaxJson();
		j.setSuccess(true);
		j.setMsg(result);
		return j;
