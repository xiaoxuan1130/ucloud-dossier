...
import com.epipe.ucloud.dossier.utils.QiniuUtil;
...
import com.jeeplus.modules.sys.enums.FileDestination;
...
//			response.setHeader("Content-Disposition", "attachment; filename=" + file.getName());
	...
	@ResponseBody
	public AjaxJson webupload( HttpServletRequest request, HttpServletResponse response,MultipartFile file) throws Exception {
        AjaxJson j = new AjaxJson();
		String uploadPath = request.getParameter("uploadPath");
		String target = request.getParameter("target");
		if (StringUtils.isEmpty(target)){
            uploadToLocal(file, j, uploadPath);
            return j;
        }
		if (FileDestination.valueOf(target) == FileDestination.qiniu) {
			uploadToQiniu(file, j);
		} else {
			uploadToLocal(file, j, uploadPath);
		}
		return j;
     ...
     * @throws IOException
     */
    private void uploadToLocal(MultipartFile file, AjaxJson j, String uploadPath) throws IOException {
     ...
     * @param j
     */
    private void uploadToQiniu(MultipartFile file, AjaxJson j) throws Exception{
        String[] arr = file.getOriginalFilename().split("\\.");
        //String name = arr[0] + IdUtil.getLongId() + "." + arr[1];
		//使用当前时间戳作为文件标识,后面好截取文件
		String name = arr[0] + DateUtils.getDate("yyyyMMddhhmmss")+ "." + arr[1];
		String url = QiniuUtil.uploadFileWithExt(file.getInputStream(), name);
		url = url +"?attname="+ URLEncoder.encode(name, "utf-8");
