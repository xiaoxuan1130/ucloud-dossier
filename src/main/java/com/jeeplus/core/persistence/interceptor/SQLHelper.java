...
public class SQLHelper {
    ...
    public static void setParameters(PreparedStatement ps, MappedStatement mappedStatement, BoundSql boundSql, Object parameterObject, Log log) throws SQLException {
...
//        final String countSql = "select count(1) " + removeSelect(removeOrders(sql));
        ...
        try {
            ...
            SQLHelper.setParameters(ps, mappedStatement, countBS, parameterObject, log);
            rs = ps.executeQuery();
            int count = 0;
            if (rs.next()) {
                count = rs.getInt(1);
