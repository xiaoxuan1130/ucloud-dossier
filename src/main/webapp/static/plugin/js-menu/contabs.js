...
function menuItem() {
    ...
    $('.J_menuTab').each(function () {
        if ($(this).data('id').indexOf(dataUrl) !== -1) {
            if (!$(this).hasClass('active')) {
            ...
            if (dataUrl.indexOf(top.$(this).data('id')) !== -1) {// 选项卡已存在，激活。
                if (!top.$(this).hasClass('active')) {
                    ...
                    top.$('.J_mainContent .J_iframe').each(function () {
                        // debugger;
                        if (dataUrl.indexOf(top.$(this).data('id')) !== -1) {
...
//打开或者刷新tab
//create by gongtao
function openOrRefreshTab(url, title) {
    var dataUrl = url;
    if (url.indexOf("?") !== -1){
    ...
    var target = $('.J_iframe[data-id^="' + url + '"]');
    if (target.length !== 0){
        var $element = top.$('.J_menuTab').filter(function () {
            return $(this).data('id').indexOf(url) !== -1;
        }).data('id', dataUrl);
        if (!$element.hasClass('active')) {
            // 显示tab对应的内容区
            $('.J_mainContent .J_iframe').each(function () {
                if ($(this).data('id').indexOf(url) !== -1) {
                    $(this).data('id', dataUrl).show().siblings('.J_iframe').hide();
                }
            });
            $element.addClass('active').siblings('.J_menuTab').removeClass('active');
            scrollToTab($element.get(0));
            //显示loading提示
            var index = layer.load(1, {
                shade: [0.1, '#fff'] //0.1透明度的白色背景
            });
