	 ...
	 //   },
	  ...
	  //获取字典标签
		...
		*    beformSubmit：执行提交操作前执行的方法
        ...
        openSaveDialogAndRefresh: function(title,url,width,height,callback){
            var auto = true;//是否使用响应式，使用百分比时，应设置为false
            if(width.indexOf("%")>=0 || height.indexOf("%")>=0 ){
                auto =false;
            }
            top.layer.open({
                ...
                yes: function(index, layero){
                    var iframeWin = layero.find('iframe')[0]; //得到弹出的窗口对象，执行窗口内iframe页的方法：iframeWin.method();
                    iframeWin.contentWindow.save(callback);//调用保存事件，在 弹出页内，需要定义save方法。处理保存事件。
                },
                cancel: function(index){
                },
        ...
        closeByName:function(name){
		    top.closeTabByName(name);
        },
        //create by gongtao
        openOrRefreshTab:function(url, title){
            top.openOrRefreshTab(url, title);
        },
        /**
		 * Ajax上传文件
         * @param fileObj
