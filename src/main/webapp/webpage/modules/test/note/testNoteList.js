...
$(document).ready(function() {
	$('#testNoteTable').bootstrapTable({
		  ...
		  //请求方法
			...
			,{
		        ...
		        ,formatter:function(value, row , index){
				   ...
				   <c:choose>
					   <c:when test="${fns:hasPermission('test:note:testNote:edit')}">
					      return "<a href='javascript:edit(\""+row.id+"\")'>"+value+"</a>";
				      </c:when>
					  <c:when test="${fns:hasPermission('test:note:testNote:view')}">
	  ...
	  $('#testNoteTable').on('check.bs.table uncheck.bs.table load-success.bs.table ' +
            ...
            $('#view,#edit').prop('disabled', $('#testNoteTable').bootstrapTable('getSelections').length!=1);
        });
		...
		$("#btnImport").click(function(){
			jp.open({
			    ...
			    btn2: function(index, layero){
						...
						iframeWin.contentWindow.importExcel('${ctx}/test/note/testNote/import', function (data) {
							if(data.success){
								jp.success(data.msg);
								refresh();
							}else{
	  ...
	  $("#export").click(function(){//导出Excel文件
			jp.downloadFile('${ctx}/test/note/testNote/export');
	  });
