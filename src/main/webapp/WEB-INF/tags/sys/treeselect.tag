...
$(document).ready(function(){
	$("#${id}Button, #${id}Name").click(function(){
		...
		var url = "${ctx}/tag/treeselect?url="+encodeURIComponent("${url}")+"&module=${module}&checked=${checked}&extId=${extId}&isAll=${isAll}&allowSearch=${allowSearch}";
		if ("${paramName}"){
			if (!eleValue){
				top.layer.msg("请先选择父项！");
				return;
			}
			url += "&${paramName}=" + eleValue;
		}
		// 正常打开	
		top.layer.open({
		    type: 2, 
		    ...
		    btn: ['确定', '关闭']
    	       ,yes: function(index, layero){ //或者使用btn1
						...
						for(var i=0; i<nodes.length; i++) {//<c:if test="${checked && notAllowSelectParent}">
							...
							var parentName = nodes[i].original.parentName;
							ids.push(nodes[i].id);
							if (null != parentName && "#" != parentName) {
                                names.push(parentName + "->" + nodes[i].text);//<c:if test="${!checked}">
